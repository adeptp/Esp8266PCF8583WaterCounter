...
void PCF8583::setMemoryLong(uint8_t offset,uint32_t count) {
    Wire.beginTransmission(_address);
    Wire.write(offset);
...
uint32_t PCF8583::getMemoryLong(uint8_t offset) {
    Wire.beginTransmission(_address);
    Wire.write(offset);
    Wire.endTransmission();
    ...
    uint32_t count = 0;
    count = Wire.read();
    count = count + (Wire.read() <<8);
    count = count + (Wire.read() << 16);
    count = count + (Wire.read() << 24);
    return count;
}

void PCF8583::setMemoryInt(uint8_t offset,uint16_t count){
    Wire.beginTransmission(_address);
    Wire.write(offset);
    Wire.write(count & 0xFF);
    Wire.write((count >> 8) & 0xFF);
...
uint16_t PCF8583::getMemoryInt(uint8_t offset) {
    Wire.beginTransmission(_address);
    Wire.write(offset);
    Wire.endTransmission();
...
void PCF8583::setMemoryByte(uint8_t offset,uint8_t count){
    Wire.beginTransmission(_address);
    Wire.write(offset);
    Wire.write(count);
    Wire.endTransmission();
...
uint8_t PCF8583::getMemoryByte(uint8_t offset){
    Wire.beginTransmission(_address);
    Wire.write(offset);
    Wire.endTransmission();
